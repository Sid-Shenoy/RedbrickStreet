 ...
 * - We match the plot-region example in `web/src/world/houseModel/requirements.txt`.
 * - All regions from the example are generated for every house, using lot-local coordinates.
 * - zsize is currently fixed at 30 (per spec).
 * - Polygons are supported and used for backyard, houseregion, and nearfrontlawn.
 *
 * Later steps can reintroduce seeded variation while preserving region coverage + naming.
 */
